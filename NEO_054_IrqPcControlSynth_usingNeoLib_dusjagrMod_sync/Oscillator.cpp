...
********************************** list of outhors **********************************
  ...
  v0.2  21.3.2017 C. -H-A-B-E-R-E-R-  SAWTOOTH waveform inverted ( fast attack )
...
void Oscillator::setWaveformFine(uint16_t wert)
{
	...
	if (wert > 168*FACTOR) wave = SAWTOOTH;
	if (wert > 220*FACTOR) wave = TRIANGLE;
	if (wert > 240*FACTOR) wave = SINUS;
