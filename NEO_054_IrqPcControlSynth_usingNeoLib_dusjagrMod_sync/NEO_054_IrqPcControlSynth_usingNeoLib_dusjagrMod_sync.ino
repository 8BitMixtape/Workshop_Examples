...
void setup()
{
  ...
  // show leds
  setColorAllPixel(pixels.Color(15, 0, 15));
  pixels.show(); // This sends the updated pixel color to the hardware.
  ...
  StartTime_ms = millis();
  oldPotiLeft = getPoti(POTI_RIGHT);
  oldPotiRight = getPoti(POTI_LEFT);
}

  int16_t lp = getPoti( POTI_RIGHT );   // range 0..1023
  int16_t rp = getPoti( POTI_LEFT );  // range 0..1023
uint8_t wheelColor = 0;
uint8_t inputMode = 30;
...
{
  if (inputMode == 0) {
    setColorAllPixel(pixels.Color(20, 0,0));
    pixels.setPixelColor( ButtonSelector1, COLOR_RED );
  }
  if (inputMode == 1) {
    setColorAllPixel(pixels.Color(20, 0,20));
    pixels.setPixelColor( ButtonSelector1, COLOR_PINK );
  }
  
  

  displayBinaryValue( 0b11100000 ,  pixels.Color(30, 20, 10)); // display the value as binary on 8 NEO leds
  displayBinaryValue( 0b00011100 ,  pixels.Color(2, 12, 22)); // display the value as binary on 8 NEO leds 

  //pixels.setPixelColor(ButtonSelector1, Wheel(lp>>2));
  if(ButtonSelector2<3)  pixels.setPixelColor(7 - ButtonSelector2, COLOR_YELLOW);
  else pixels.setPixelColor(7 - ButtonSelector2, COLOR_CYAN);
...
void loop()
{
  ...
  lp = getPoti( POTI_RIGHT );   // range 0..1023
  rp = getPoti( POTI_LEFT );  // range 0..1023
  wheelColor = lp>>2;
    ...
    if ( abs(lp - oldPotiLeft) > POITHYSTERESIS )
    {
      switch (ButtonSelector1)
      {
        case 0:
          {
            HFOfrequency = lp/4;
            setFrequency(HFOfrequency);
          }
          break;
        case 1:
          {
            setWaveformFine(lp / 4);
          }
          break;
          /*
        case 2:
        ...
        case 0:
          {
            ...
            if (LFO1_Frequency_mHz <= 0) LFO1_Frequency_mHz = 0;
            lfo1.setFrequency_mHz( LFO1_Frequency_mHz );
  ...
  if (LFO2_Frequency_mHz != 0  ) 
  {
    setAmplitude((v2 * Volume) >> 8);
